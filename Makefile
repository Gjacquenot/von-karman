...
# Libraries
LIBS := -lm -L/usr/lib -lhdf5_cpp -lhdf5
INCLUDES := -I/usr/include #-I/path/to/eigen3
...
$(BIN)/%.o: $(SRC)/%.cpp
	@$(CXX) $(CXXFLAGS) -I$(INCLUDE) -I$(INCLUDES) -c $< -o $@ $(LIBS)

# We compile the .c files
$(BIN)/%.o: $(SRC)/%.c
	@$(CC) $(CFLAGS) -I$(INCLUDE) -I$(INCLUDES) -c $< -o $@ $(LIBS)
