...
def upwind(u, v, q):
    ...
    # Matrices where 1 is right, 0 is left or center
    Mx2 = np.sign(np.sign(u[1:-1, 1:-1]) + 1.)  # 1 if u > 0, 0 if u < 0
    Mx1 = 1. - Mx2  # 1 if u < 0, 0 if u > 0

    # Matrices where 1 is up, 0 is down or center
    My2 = np.sign(np.sign(v[1:-1, 1:-1]) + 1.)
    My1 = 1. - My2

    ADVq[1:-1, 1:-1] = q[1:-1, 1:-1] - dt * (u[1:-1, 1:-1] * (
        Mx2 * (q[1:-1, 1:-1] - q[:-2, 1:-1]) +
        Mx1 * (q[2:, 1:-1] - q[1:-1, 1:-1])
    ) / dx +
        v[1:-1, 1:-1] * (
        My2 * (q[1:-1, 1:-1] - q[1:-1, :-2]) +
        My1 * (q[1:-1, 2:] - q[1:-1, 1:-1])
    ) / dy)
