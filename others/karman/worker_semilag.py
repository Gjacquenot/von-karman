...
def update_pressure(ustarstar, vstarstar, rho, epsilon, dx, dy, nx_sp, ny_sp, K, dt, bc):
...
def run_simulation():
    ...
    kx = np.append(kx, np.array([(2 * np.pi * (NX- i) / lx) for i in range(int(NX / 2) - 1, NX)]))
    ...
    print(f"Solution saved to: {folder_path}")
    end_time = time.time()
