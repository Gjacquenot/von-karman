...
def step1(u, v, nx, ny, nu, x, y, xx, yy, dx, dy, dt, epsilon, F, R, theta, r, uRHS_conv_diff_p, uRHS_conv_diff_pp,
    ...
    ibm_forcing_u, ibm_forcing_v = ibm.circle(
        uRHS, vRHS, u, v, dt, x, y, xx, yy, r, R, theta, nx, ny, F, epsilon)
    uprim = Semilag2(u, v, u, dx, dy, dt, nx, ny)
    vprim = Semilag2(u, v, v, dx, dy, dt, nx, ny)
    ustar = u + F * dt + ibm_forcing_u * dt + dt * uRHS
...
def Semilag_t(u, v, q, dx, dy, dt, NX, NY):
    """
    1st order semi-Lagrangian advection
    """
    ADVq = np.zeros_like(q)
    ...
    # Matrices where 1 is right, 0 is left or center
    Mx2 = np.sign(np.sign(u[j_min:j_max, i_min:i_max]) + 1.)
    Mx1 = 1. - Mx2

    # Matrices where 1 is up, 0 is down or center
    My2 = np.sign(np.sign(v[j_min:j_max, i_min:i_max]) + 1.)
    My1 = 1. - My2

    # Matrices of absolute values for u and v
    au = abs(u[j_min:j_max, i_min:i_max])
    av = abs(v[j_min:j_max, i_min:i_max])
    ...
    # Computes the advected quantity for the interior (central domain)
    ADVq[j_min:j_max, i_min:i_max] = (Cc * q[j_min:j_max, i_min:i_max] +
                                      Ce * (Mx1 * My1 * q[j_min + 1:j_max + 1, i_min + 1:i_max + 1] +
                                            Mx2 * My1 * q[j_min + 1:j_max + 1, i_min - 1:i_max - 1] +
                                            Mx1 * My2 * q[j_min - 1:j_max - 1, i_min + 1:i_max + 1] +
                                            Mx2 * My2 * q[j_min - 1:j_max - 1, i_min - 1:i_max - 1]) +
                                      Cmx * (My1 * q[j_min + 1:j_max + 1, i_min:i_max] +
                                             My2 * q[j_min - 1:j_max - 1, i_min:i_max]) +
                                      Cmy * (Mx1 * q[j_min:j_max, i_min + 1:i_max + 1] +
                                             Mx2 * q[j_min:j_max, i_min - 1:i_max - 1]))
